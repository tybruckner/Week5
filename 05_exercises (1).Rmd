...
```{r, echo=FALSE}
knitr::include_graphics("trains_Paris.gif")
```
...
```{r,echo=FALSE}
knitr::include_graphics("tomato_1.gif")
```
...
```
  ...
  <br />
  I prefer the animated graph because it feels like a bike ride instead of many different points plotted. It is easy to tell where the ride started.
...
```
```{r,echo=FALSE}
knitr::include_graphics("panama_1.gif")
```
<br />
...
```{r,echo=FALSE}
knitr::include_graphics("covid_19.gif")
```
  ...
  Overall the graph is messy hard to read. The feature I really like is that the state name with the most new cases will rise to the top when they were a COVID hotspot. Slowing down the gif also makes it more readable. We also do not account for population density in this graph. Early in the pandemic states with high density rose to the top.
...
```{r, echo=FALSE}
knitr::include_graphics("us_map_covid.gif")
```


It is very interesting that cases per 10000 has generally evened out by our current time. Initially the coasts had the first cases. I really like this map and find it informative. I also think that testing capacity was a major difference in the amount of cases in different states at the beginning of the pandemic.
